...
const userEmail = document.querySelector('.user-info-container').id;
...
function load_posts(lastArticleNo){
    ...
    xhr.open('GET', `/contents?lastPostNo=${lastArticleNo}&count=6&email=${userEmail}`);
    xhr.send(null); // GET 요청에서는 body가 null이여야 함
}

/************************************************************/
function follow_user(userEmail){
    fetch(`/user/follow`, {
        method: "POST",
...
function unFollow_user(userEmail){
    fetch(`/user/follow`, {
        method: "DELETE",
        headers: {"X-CSRF-TOKEN": csrfToken},
        body: userEmail
